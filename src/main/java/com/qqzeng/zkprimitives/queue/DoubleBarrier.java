...
/**
 * DoubleBarrier, which is corresponding to Double Barrier in paper.
 * In short, before the client does some work, it must enter the barrier,
 * and wait until all clients do. Before the client try to leave the barrier
 * after it finishes work, it has to wait until all clients do.
 * <p>
 *
 *  About Test:
 ...
 *       5. Try to synchronize leave the barrier after doing some work.
 *       6. one round test over, continue until reaching to NUM_ROUND times.
 *
 */
public class DoubleBarrier {
    private static final Logger LOGGER = Logger.getLogger(DoubleBarrier.class);
    ...
    private static final String ROOT = "/queue-double-barrier";
    ...
    public DoubleBarrier(int nodeNum, String hostport, int nodeCnt) {
    ...
    private void deleteNode() throws InterruptedException, KeeperException {
        zk.delete(this.myZnode, -1);
    }

    private boolean judgeSynchronizeArrive() throws KeeperException, InterruptedException {
        List<String> list = zk.getChildren(ROOT, false);
        return list.size() == this.nodeCnt;
    }

    private boolean judgeSynchronizeLeave() throws KeeperException, InterruptedException {
        List<String> list = zk.getChildren(ROOT, false);
        return list.size() == 0;
    }

    private void synchronizeNode(String action) throws KeeperException, InterruptedException {
        zk.getChildren(ROOT, true);
        synchronized (this.mutex) {
            LOGGER.info(this.logPrefix + " waits for queue "+ action +" synchronizing.");
            mutex.wait();
        }
    }
    ...
    public void synchronizeQueue() throws KeeperException, InterruptedException {
        createNode();
        while (!judgeSynchronizeArrive()) {
            synchronizeNode("ARRIVE");
        }
        LOGGER.info(this.logPrefix + " enter the double barrier.");
        doWork();
        deleteNode();
        while (!judgeSynchronizeLeave()) {
            synchronizeNode("LEAVE");
        }
        LOGGER.info(this.logPrefix + " leave the double barrier.");
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < NUM_ROUND; i++) {
            ...
            DoubleBarrier b = new DoubleBarrier(nodeNum, CONNECT_STRING, nodeCnt);
            try {
                b.synchronizeQueue();
            } catch (Exception e) {
                e.printStackTrace();
